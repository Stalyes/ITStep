...
def trains(request):
    ...
    return render(request, "trains.html", {
        "trains": found, "date": day, "from_name": src, "to_name": dst, "num_tickets": n
    })
...
def process_payment(request):
    if request.method == "POST":
        ...
        for p in plist:
            seat_id = int(p["seatId"]) if p.get("seatId") else None
